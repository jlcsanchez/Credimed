...
## Anonymous-first signup flow (deferred signup wall)

**Status:** Rejected. Don't build.

**Idea:** Reorder the funnel so users upload docs and see their
precise refund estimate BEFORE being asked to sign up. Common growth
pattern (TurboTax pre-2020, Mint, etc.).

**Why we rejected:** The landing already has an interactive estimator
that gives users a reasonable refund range without signup. The
marginal value of going from "rough estimate on landing" to "precise
estimate without signup" is much smaller than the cost of building
anonymous S3 uploads + HIPAA exposure + abuse mitigation +
state-migration logic. ROI doesn't justify risk at our scale.

**Trigger to revisit:** If conversion from landing-estimator to
signup drops below 30% post-launch AND we have evidence that users
specifically bounce because they want a precise number, reconsider.
Until then, optimize email-recovery-on-abandon and pre-fill the flow
with landing inputs (city, procedure) — those are higher ROI.
