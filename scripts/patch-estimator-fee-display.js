 ...
 * Plus $79 / Premium $99) and applies the 20% cap internally (the cap
 * number is intentionally NOT surfaced in the UI — it just keeps the
 * fee proportional on small refunds without adding cognitive load).
 */
const fs = require('fs');
...
const newMath = `  const lowEnd  = Math.max(50, Math.round(effectivePaid * 0.60));
  ...
  // big, the cap is moot (fee stays at tier). The cap percentage is
  // intentionally NOT surfaced in the UI — it just keeps the fee
  // proportional without adding cognitive load.
